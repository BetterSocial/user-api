...
const BetterSocialCreatePost = async (req, isAnonimous = true) => {
  ...
  try {
    ...
    const { tagUsers } = req.body;
    if (Array.isArray(tagUsers)) {
      tagUsers.forEach((userId) => {
        (async () =>
          await fcmToken.sendMultiDeviceTaggedInPostNotification(
            userId,
            userDetail,
            body.message,
            post.id
    ...
    await insertTopics(filteredTopics);
