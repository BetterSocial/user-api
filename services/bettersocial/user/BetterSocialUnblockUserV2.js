...
const BetterSocialUnblockUserV2 = async (token, selfUserId, targetUserId) => {
    ...
    try {
        await sequelize.transaction(async (t) => {
            await UserBlockUserFunction.userUnblock(UserBlockedUser, selfUserId, targetUserId, { transaction: t })
        });
    } catch (e) {
    ...
    try {
        ...
        await Getstream.feed.followAnonUser(token, selfUserId, targetUserId, selfAnonymousUserId?.user_id, targetAnonymousUserId?.user_id)
        return {
            isSuccess: true,
