...
const getCredderScoreCache = async (domainPageId) => {
  ...
  if (credderScoreCache) {
    ...
    dataCached.credderLastChecked = await RedisDomainHelper.getDomainCredderLastChecked(
      domainPageId
    );
  } else {
...
const activityFormatter = async (item, feedGroup, userId, excludedPostParameter) => {
  const {listAnonymousAuthor, listBlock, myLocations, listAnonymousPostId} = excludedPostParameter;
