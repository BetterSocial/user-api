...
const handle_anon_to_anon_channel_member = async (
  ...
  console.log('DEBUG channel_owner => ', channel_owner);
  console.log('DEBUG oldChannelId => ', oldChannelId);
  // check if channel already exist
  let channel = await ChatAnonUserInfo.findOne({
    where: {
      my_anon_user_id: member,
...
module.exports = {
  ...
  findOrCreateChannel: async (req, res) => {
    ...
    try {
      ...
      if (is_anon_to_anon) {
        // check if channel already exist
        let new_channel = await handle_anon_to_anon_channel_owner(req.userId, members[0]);
        await handle_anon_to_anon_channel_member(new_channel, members[0], oldChannelId);
