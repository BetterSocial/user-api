...
module.exports = async (req, res) => {
  ...
  const response = await getDetailFeed(postId);
  ...
  if (isDateExpired(feed?.expired_at))
  ...
  if (feed?.actor?.id === req?.userId)
    return res?.status(200)?.json({
      success: true,
      message: 'You are the author of this feed'
    });

  const isAuthorBlockedMe = await UserBlockedUser?.findOne({
    where: {
  ...
  const isAuthorFollowMe = await UserFollowUser?.findOne({
    where: {
