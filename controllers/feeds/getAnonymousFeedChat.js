...
const mappingFeed = (req, feeds) => {
  const mapping = feeds.activities.map((feed) => {
    return {
      ...feed,
...
const finalize = (
  ...
  if (myReaction) {
    myReaction = {
      ...myReaction,
      isOwningReaction:
        req.userId === myReaction.user_id || myReaction.user_id === mySignedId,
    };
    if (
...
const getDetail = (req, b, mySignedId) => {
  ...
  const isOwnPost = actor.id === req.userId || actor.id === mySignedId;
  if (isAnonym) {
    actor = {
      ...actor,
      id: null,
