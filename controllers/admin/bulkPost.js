...
const bulkPostController = async (req, res) => {
  try {
    ...
    for (let element of post) {
      ...
      req.userId = userId;
      req.token = token;
      let {isSuccess, message} = await BetterSocialCore.post.createPost(req, anonimity);
      if (!isSuccess) {
        console.log(message);
...
const generateAnonymousUsername = async () => {
  ...
  try {
    ...
    while (body?.contentType === CONTENT_TYPE_COMMENT && currentIteration <= maxIteration) {
      const anotherAnonUserInfo = await PostAnonUserInfoFunction.checkAnotherUsernameInPost(
        PostAnonUserInfo,
        User,
        {
          postId: body?.postId,
