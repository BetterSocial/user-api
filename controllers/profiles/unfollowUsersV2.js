...
module.exports = async (req, res) => {
    ...
    if (!await UserFollowUserFunction.checkIsUserFollowing(UserFollowUser, req?.userId, user_id_followed)) return ErrorResponse.e409(res, "You have not followed this user")

    try {
        await sequelize.transaction(async (t) => {
            const userFollowUser = await UserFollowUserFunction.userUnfollow(UserFollowUser, UserFollowUserHistory,req?.userId, user_id_followed, follow_source, t)
    ...
    try {
        ...
        const unfollowMainFeedFollowing = Getstream.feed.unfollowMainFeedFollowing(req?.token, req?.userId, user_id_followed);
        
        await Promise.all([unfollowUserExcl, unfollowUser, unfollowMainFeedFollowing]);
