...
const blockUserV2 = async (req, res) => {
    ...
    const response = await BetterSocialCore.user.blockUser(token, selfUserId, targetUserId, source, { postId, message, reason })
    if(response?.isSuccess) return SuccessResponse(res, {}, response?.message || "User has been blocked successfully")
